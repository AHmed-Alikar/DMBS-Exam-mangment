...
Select StdName, max(Total) As Test_Total
	from S2_BIT5_midterm
	group by StdName
	Having  sum(Total) > 120
...
create index roww on S2_BIT5_final (StdName Asc)

sp_helpindex S2_BIT5_final

(StdName varchar(30) Foreign key references S2_BIT5_final (stdId) )
...
select StdName, MAX(FTotal)from S2_BIT5_final
group by StdName
having max (FTotal)>300;
